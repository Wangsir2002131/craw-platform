...
try:
    from shared_methods import (
        ...
        get_proxy, dp_api_yuanbao, send_dingtalk_message, DB_CONFIG
    )
except Exception:
    DB_CONFIG = {}
    dp_api_yuanbao = None
...
class YuanBao:
    ...
    def get_loginFile(self, cookie_file):
        """获取 cookie 文件路径"""
        return get_loginFile(self.currentPath, 'yuanbao_cookie_file', cookie_file)
    ...
    def login(self, page: Page):
        """登陆"""
        page.goto(r'https://yuanbao.tencent.com/chat/', wait_until="load", timeout=30000)
        self.wait_load_page(page)
        logger.info("✓ 已导航到 元宝 主页，等待登陆...")
        page.query_selector('text=联网搜索').click()
            ...
            try:
                with sync_playwright() as p:
                    ...
                    try:
                        ...
                        try:
                            self.page.goto('https://yuanbao.tencent.com/chat/', wait_until='domcontentloaded')
                            self.page.wait_for_timeout(30000)
                            zdss = self.page.query_selector('text=自动搜索')
                            if zdss:
                                zdss.click()
                                self.page.wait_for_timeout(3000)
                            sd = self.page.query_selector('text=手动')
                            if sd:
                                sd.click()
                                self.page.wait_for_timeout(3000)
                            lwss = self.page.query_selector('text=联网搜索')
                            if lwss:
                                lwss.click()
                                self.page.wait_for_timeout(3000)
                            # sdsk = self.page.query_selector('text=深度思考')
                            # if sdsk:
                            #     sdsk.click()
                            #     self.page.wait_for_timeout(3000)
                        except:
    ...
    def ask_one_question_with_retry(self, question_id: str, question_name: str):
        ...
        all_cookie_files = self.get_available_cookie_files(self.currentPath, "yuanbao_cookie_file", self.cookie_file)
    ...
    def _interact_with_page_core(self, question_id: str, question_name: str):
        ...
        try:
            # 找到输入框
            div_box = self.page.query_selector('[data-placeholder="有问题，尽管问，shift+enter换行"]')

            if div_box:
                # 输入问题
                div_box.fill(f'{question_name} 帮我联网检索最新信息')
                self.page.wait_for_timeout(2000)
                # 找到提交按钮
                submit_button = self.page.query_selector('#yuanbao-send-btn')
                ...
                if submit_button:
                    ...
                    # # 等待回答（30-50秒）
                    self.page.wait_for_timeout(random.randint(40000, 50000))
                    # 等待回答（60-90秒）
                    # self.page.wait_for_timeout(random.randint(60000, 90000))

                    # 检查回答是否完整
                    div_value = self.page.query_selector_all('.hyc-common-markdown.hyc-common-markdown-style')
                    if div_value:
                        while True:
                            # 在循环内部获取最新的元素和文本
                            ...
                            if visible_text == div_value_text:
                                logger.info('✅ 检测到回答完整')
                                dt_button_id = self.page.query_selector('.agent-chat__scroll-arrow.agent-chat__scroll-arrow--show')
                                if dt_button_id:
                                    dt_button_id.click()
                    ...
                    # 获取答案内容
                    target_div = self.page.query_selector_all('.hyc-common-markdown.hyc-common-markdown-style')
                    visible_text = target_div[-1].inner_text()
                    executor = ThreadPoolExecutor(max_workers=1)
                    future = executor.submit(dp_api_yuanbao, visible_text)
                    ...
                    # 查找并点击网页按钮
                    web_button = self.page.query_selector_all('#search-guide-tool')
                    if web_button:
                        web_button[-1].click()
                        self.page.wait_for_timeout(5000)

                        # 收集URL
                        result_divs = self.page.query_selector_all('.agent-dialogue-references__item')
                        url_meta_list = []
                        search_count = 0

                        for div in result_divs:
                            try:
                                href_value = div.query_selector('.hyc-common-markdown__ref_card').get_attribute("data-url")
                                href_count = div.query_selector('.hyc-common-markdown__ref_card').query_selector('.hyc-common-markdown__ref_card-foot__source_txt').text_content()
                                site_name = div.query_selector('.hyc-common-markdown__ref_card').query_selector('.hyc-common-markdown__ref_card-foot__source_txt').text_content()
                                title_element = div.query_selector('h4').text_content()
                                search_count += 1
                                url_meta_list.append((href_value, href_count, title_element, search_count, site_name))
                            except Exception as e:
                                logger.error(f"处理搜索结果失败: {e}")

                        logger.info(f"✅ 收集到 {len(url_meta_list)} 个URL，已提交到后台线程处理")

                        # 🚀 在完全独立的后台线程中处理所有URL（不阻塞主进程）
                        if len(url_meta_list) > 0:
                            import threading
                        ...
                        logger.error(f"没有找到 data-yb-exposured 的 div 元素")
                else:
                ...
                with sync_playwright() as p:
                    ...
                    try:
                        ...
                        try:
                            ...
                            zdss = page.query_selector('text=自动搜索')
                            if zdss:
                                zdss.click()
                                page.wait_for_timeout(3000)
                            sd = page.query_selector('text=手动')
                            if sd:
                                sd.click()
                                page.wait_for_timeout(3000)
                            lwss = page.query_selector('text=联网搜索')
                            if lwss:
                                lwss.click()
                                page.wait_for_timeout(3000)
                            # sdsk = page.query_selector('text=深度思考')
                            # if sdsk:
                            #     sdsk.click()
                            #     page.wait_for_timeout(3000)
                        except:
                        ...
                        # 创建临时实例执行交互
                        temp_instance = YuanBao(cookie_file=cookie_file, proxy_api=self.proxy_api)
    ...
    def _process_urls_in_background(self, question_id: str, url_meta_list: list):
        ...
        Args:
            question_id: 问题ID
            url_meta_list: URL元数据列表 [(href, count, name, title, count, site_name), ...]
        """
        try:
            ...
            # 按顺序处理并插入数据库（不获取网页内容）
            for idx, (href_value, href_count, title_element, count, site_name) in enumerate(url_meta_list):
                try:
                    logger.info(f"[{thread_name}][{idx+1}/{len(url_meta_list)}] 引用链接地址:{href_count} {href_value}")
                    logger.info(f"[{thread_name}] 引用链接标题:{title_element}")
...
def process_cookie_task(cookie_file: str, question: Dict[str, Any], round_num: int, proxy_api: Optional[Callable] = None):
    ...
    try:
        ...
        yb = YuanBao(cookie_file=cookie_file, proxy_api=proxy_api)
        yb.run_single(question, round_num)
...
def run_concurrent(max_workers: int = 2, proxy_api: Optional[Callable] = None, questions: Optional[List[Dict[str, Any]]] = None):
    ...
    """
    yuanbao = YuanBao()
    folder_path = os.path.join(yuanbao.currentPath, "yuanbao_cookie_file")
    ...
    try:
        if questions is None:
            ...
            questions_list = [YuanBao.normalize_question_task(question) for question in questions]
        ...
        tasks = YuanBao.build_round_interleaved_tasks(questions_list, cookie_files)
        if not tasks:
            logger.error("没有生成可执行任务，程序终止。")
            return

        schedule_preview = ", ".join(f"{task['QuestionId']}-r{round_num}" for _, task, round_num in tasks
        )
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # 提交所有任务到线程池，线程池会自动调度
            futures = {
                executor.submit(process_cookie_task, cookie_file, question, round_num, proxy_api):(cookie_file, question, round_num)
...
def main_sync():
    ...
    # 从数据库获取问题
    questions = get_questions_from_db()
    yuanbao.run(questions=questions)
...
def execute_task(task_info: dict, account_info: dict = None) -> dict:
    """Execute one YuanBao question task for the master dispatcher."""
    if account_info:
