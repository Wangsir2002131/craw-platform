...
3	account_error_rate	error_rate_rule(threshold=0.3)	🔴 RED	账号操作错误率 > 30%
🔹 队列规则 QueueAlertRule — 3条
#	规则名	工厂方法	级别	触发条件
...
6	redis_connection_error	redis_connection_error_rule()	🔴 RED	Redis 连接失败
🔹 系统规则 SystemAlertRule — 2条
#	规则名	工厂方法	级别	触发条件
7	memory_usage_high	memory_high_rule(threshold=0.8)	🔴 RED	内存使用率 > 80%
8	database_connection_failure	db_connection_failure_rule()	🔴 RED	数据库 SELECT 1 探测失败
🔹 任务规则 TaskAlertRule — 2条
#	规则名	工厂方法	级别	触发条件
9	task_timeout	timeout_rule(timeout_seconds=300)	🟡 YELLOW	任务执行 > 300 秒未完成
10	task_failure_rate	failure_rate_rule(threshold=0.1)	🔴 RED	任务失败率 > 10%
"""