#!/usr/bin/env python
"""Run the AFu Redis consumer directly."""
...
from platform.consumers.afu_consumer import AfuConsumer  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run AFu Redis consumer.")
...
def main() -> int:
    ...
    logging.getLogger(__name__).info(
        "starting afu consumer runner: queue=%s priority_queue=%s timeout=%s idle_sleep=%s",
        get_queue_name("afu"),
        f"{get_queue_name('afu')}:priority",
        args.timeout,
        args.idle_sleep,
    )
    if args.once:
        consumer = AfuConsumer(DB_CONFIG)
        processed = consumer.run(
            once=True,
            timeout=args.timeout,
            idle_sleep=args.idle_sleep,
        )
        print(f"afu consumer stopped, processed={processed}")
        return 0
    supervisor = ConsumerSupervisor(
        model_key="afu",
        consumer_factory=lambda: AfuConsumer(DB_CONFIG),
        timeout=args.timeout,
        idle_sleep=args.idle_sleep,
    )
