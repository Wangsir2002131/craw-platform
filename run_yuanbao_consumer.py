#!/usr/bin/env python
"""Run the YuanBao Redis consumer directly."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run YuanBao Redis consumer.")
...
def main() -> int:
    ...
    logging.getLogger(__name__).info(
        "starting yuanbao consumer runner: queue=%s priority_queue=%s timeout=%s idle_sleep=%s",
        get_queue_name("yuanbao"),
        f"{get_queue_name('yuanbao')}:priority",
        args.timeout,
        args.idle_sleep,
    )
    if args.once:
        consumer = YuanbaoConsumer(DB_CONFIG)
        processed = consumer.run(
            once=True,
            timeout=args.timeout,
            idle_sleep=args.idle_sleep,
        )
        print(f"yuanbao consumer stopped, processed={processed}")
        return 0
    supervisor = ConsumerSupervisor(
        model_key="yuanbao",
        consumer_factory=lambda: YuanbaoConsumer(DB_CONFIG),
        timeout=args.timeout,
        idle_sleep=args.idle_sleep,
    )
