#!/usr/bin/env python
"""Run the DouBao Redis consumer directly."""
...
from platform.consumers.doubao_consumer import DoubaoConsumer  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run DouBao Redis consumer.")
...
def main() -> int:
    ...
    logging.getLogger(__name__).info(
        "starting doubao consumer runner: queue=%s priority_queue=%s timeout=%s idle_sleep=%s",
        get_queue_name("doubao"),
        f"{get_queue_name('doubao')}:priority",
        args.timeout,
        args.idle_sleep,
    )
    if args.once:
        consumer = DoubaoConsumer(DB_CONFIG)
        processed = consumer.run(
            once=True,
            timeout=args.timeout,
            idle_sleep=args.idle_sleep,
        )
        print(f"doubao consumer stopped, processed={processed}")
        return 0
    supervisor = ConsumerSupervisor(
        model_key="doubao",
        consumer_factory=lambda: DoubaoConsumer(DB_CONFIG),
        timeout=args.timeout,
        idle_sleep=args.idle_sleep,
    )
